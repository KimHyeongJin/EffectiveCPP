...
파생 클래스 객체가 생성될 때 그 객체의 기본 클래스 부분이 파생 클래스 부분보다 먼저 호출되는 것이 정석이니까요. Transaction 생성자의 마지막 줄을 힐끗 
보면 가상 함수인 logTransaction을 호출히는 문장이 보이는데, 무척이나 당혹스러운 사건이 벌어지는 부분이 바로 이 문장입니다.여기서 호출되는 logTransaction 
함수는 BuyTransaction의 것이 아니라 Transaction의 것이란 사실! 지금 생성되는 객체의 타입이 BuyTransaction인데도 말이죠 잘 새겨두세요. 기본 클래스의 
생성자가 호출될 동안에는, 가상 함수는 절대로 파생 클래스 쪽으로 내려가지 않습니다. 그 대신, 객 체 자신이 기본 클래스 타입인 것처럼 동작합니다. 이해가 안 되세요?
기본 클래스 생성 과 정에서는 가상 함수가 먹히지 않는다고요.

그다지 직관적이지 않아 보이긴 합니다만 이 같은 동작에는 다 이유가 있습니다.
****** 아시다시피 기본 클래스 생성자는 파생 클래스 생성자보다 앞서서 실행되기 때문에 기본 클래스 생성자가 돌아가고 있을 시점에 파생 클래스 데이터 멤버는 아직 
초기화된 상태가 아니라는 것이 핵심입니다. ******

지금 해드린 이야기보다 사실 다섯 배는 더 핵심적인 이야기를 꺼낼 때가 왔습니다.
파생 클래스 객체의 기본 클래스 부분이 생성되는 동안은 그 객체의 타입은 바로 기본 클래스입니다.
호출되는 가상 함수는 모두 기본 클래스의 것으로 결정(resolve)될 뿐만 아니라, 런타임 타입 정보를 사용하는 언어 요소[이를 테면 dynamic_cast(항목 27 참조)라든지 
typeid 같은 것을 사용한다고 해도 이 순간엔 모두 기본 클래스 타입의 객체로 취급합니다. 위의 예제를 가지고 설명해 볼까요? BuyTransaction 객체의 기본 클래스 부분을 
초기화하기 위해 Transaction 생성자가 실행되고 있는 동안에는, 그 객체의 타입이 Transaction이라는 이야기 입니다. 이런 식의 처리는 C++ 언어의 다른 모든 기능에서 
이루어지고 있고, 타당성도 아주 충분합니다. BuyTransaction 클래스만의 데이터는 아직 초기화된 상태가 아니기 때문에, 아예 없었던 것처럼 취급하는 편이 최고로 안전하다는 거죠.
그러니까 파생 클래스의 생성자의 실행이 시작되어야만 그 객체가 비로소 파생 클래스 객체의 면모를 갖게 됩니다.

객체가 소멸될(소멸자가 호출될) 때는 어떻게 될까요? 똑같이 생각하면 됩니다. 파생 클래스의 소멸자가 일단 호출되고 나면 파생 클래스만의 데이터 멤버는 정의되지 않은 값으로 
가정하기 때문에, 이제부터 C++는 이들을 없는 것처럼 취급하고 진행합니다. 기본 클래스 소멸자에 진입할 당시의 객체는 더도 덜도 아닌 기본 클래스 객체가 되며, 모든 C++ 
기능들(가상함수, dynamic_cast, 기타 등등) 역시 기본 클래스 객체의 자격으로 처리합니다.

위의 예제 코드를 다시 봐주세요. Transaction 생성자에서 가상 함수를 직접 호출하는 코드가 바로 보이죠? 이번 항목의 지침을 위반한 것인데, 반론의 여지가 없을 뿐 아니라 
눈에도 잘 띄는 위반이 되겠습니다. 이런 위반은 누가 보더라도 발견이 쉽기 때문에, 이런 경우에 대해 경고 메시지를 내어 주는 컴파일러도 꽤 있습니다(물론 아닌 컴파일러도 
꽤 되지요. 컴파일러 경고에 대해 더 알고 싶은 분은 항목 53을 참고 하시면 됩니다). 뭐 컴파일러 경고가 나오지 않더라도 프로그램 실행 전에 문제가 드러날 수도 있습니다. 
logTransaction 함수가 Transaction 클래스에서 순수 가상 함수로 선언되어 있기 때문입니다. 이 함수의 정의가 존재하지 않으면(이런 일이 없을거 같죠? 그런데 가능합니다. 
항목 34 참조) 링크 단계에서 에러가 생깁니다. 링크가 제대로 되려면 Transaction::logTransaction 의 구현 코드를 찾아야 하는데, 찾아질 리가 없으니까요.

생성자 혹은 소멸자 안에서 가상 함수가 호출되는지를 잡아내는 일이 항상 쉬운 것은 아닙니다. Transaction 의 생성자가 여러 개 된다고 가정해 보세요. 각 생성자에서 하는 
일이 조금씩은 다르겠지만 몇 가지 작업은 똑같을 텐데요. 똑같은 작업을 모아 공동의 초기화 코드로 만들어 두면 코드 판박이 현상을 막을 수 있겠지요? 대개 이런 설계의 열매로 
private 멤버인 비가상 초기화 함수가 만들어지는데, 이 함수 안에서 logTransaction을 호출한다고
가정해 봅시다.
*/
...
{
public:
	Transaction() { Init(); };	// 비가상 멤버 함수를 호출하고 있...
	virtual void logTransaction() const = 0;//타입에 따라 달라지는 로그 기록을 만듭니다.
	//...
...
이 함수는 기본 클래스에 생성자 쪽으로 넘길 값을 생성하는 용도로 쓰이는 도우미 함수인데, 

*** 기본 클래스에 멤버 초기화 리스트가 만수산 드렁칡처럼 달려 있는 경우에 특히 훨씬 편리합니다(그리고 읽기에도 더 편해요). 정적 멤버로 되어 있기 때문에, 
생성이 채 끝나지 않은 BuyTransaction 객체의 미초기화된 데이터 멤버를 자칫 실수로 건드릴 위험도 없습니다. 이 대목엔 몇 번 줄을 치셔되 됩니다.
